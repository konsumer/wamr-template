...
static void test_bytes_in(wasm_exec_env_t exec_env, unsigned int *i,
                          unsigned int iLen) {}
...
static int wasm_host_load(char *filename) {
  ...
  error_buf[0] = 0;
  module_inst =
      wasm_runtime_instantiate(module, stack_size, heap_size, error_buf, 128);
  if (error_buf[0] != 0) {
...
void wasm_host_update() {
  if (cart_update != NULL) {
    ...
    if (!wasm_runtime_call_wasm_a(exec_env, cart_update, 0, NULL, 1,
                                  func_args)) {
